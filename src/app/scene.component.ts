...
@Component({
	...
	imports: [NgtArgs, Lights, Airplane, Sea, Sky],
...
export class AviatorScene {
	...
	constructor() {
		...
		injectBeforeRender(({ delta }) => {
			this.game.distance +=
				this.game.speed * delta * 1000 * this.world.ratioSpeedDistance;
			this.game.baseSpeed +=
				(this.game.targetBaseSpeed - this.game.baseSpeed) * delta * 1000 * 0.02;
			this.game.speed = this.game.baseSpeed * this.game.planeSpeed;

			if (
				Math.floor(this.game.distance) % this.world.distanceForCoinsSpawn ==
					1 &&
